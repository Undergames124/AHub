...
local success, err = pcall(function()
    ...
    if not LocalPlayer then
        warn("Ξ|Ω ESP: LocalPlayer not ready, waiting...")
    ...
    local function renderESP()
        ...
        for _, player in pairs(Players:GetPlayers()) do
            ...
            if Settings.ShowTracer then
                local centerScreen = cam.ViewportSize.X / 2
                data.tracer.Visible = true
                data.tracer.Size = UDim2.new(0, 2, 0, box.Y + box.Height)
                data.tracer.Position = UDim2.new(0, centerScreen - 1, 0, 0)
    ...
    print("Ξ|Ω ESP loaded successfully — no nil errors")
end)

if not success then
    warn("Ξ|Ω ESP failed to load: ", err)
    -- Retry after 1 second (Delta might need injection time)
    task.wait(1)
    loadstring(game:GetService("CoreGui").WaitForChild("ESP_MasterUI", math.huge) and "return" or script.Source)()
end
