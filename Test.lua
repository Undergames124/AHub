-- ============================================================
--  Ξ|Ω DYNAMIC — ESP + IMGUI MENU v5 (FIXED)
--  Исправлено:
--  - Бокс и визуалы теперь точно по росту персонажа (2.5 стада вверх/вниз от HumanoidRootPart)
--  - Скелет полный: голова -> торс -> бёдра, плечи -> кисти, бёдра -> стопы
--  - Плавность (33 FPS)
--  - Шляпа теперь опционально и на LocalPlayer
-- ============================================================
...
local Settings = {
    ...
    ShowOnLocal = false,  -- Показывать ли визуалы на себе (шляпу, бокс итд)
}
...
local function CreateToggleButton()
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        ...
        if MenuFrame then MenuFrame.Visible = MenuVisible end
    end)
...
local function CreateMenu()
    ...
    MenuFrame.Size = UDim2.new(0, 450, 0, 400)
    MenuFrame.Position = UDim2.new(0.5, -225, 0.5, -200)
    ...
    local function ShowVisualsTab()
        ...
        CreateCheckbox("Show on LocalPlayer", "ShowOnLocal", y); y = y + 32
...
-- ---- ESP (FULL REWRITE с исправлениями) ----
local function CreateESPObjects(player)
    ...
    local cache = {
        ...
        -- Скелет
        HeadToTorso = MakeDrawing("Line"),
        TorsoToHips = MakeDrawing("Line"),
        LeftArm = MakeDrawing("Line"),
        RightArm = MakeDrawing("Line"),
        LeftLeg = MakeDrawing("Line"),
        RightLeg = MakeDrawing("Line"),
        -- Шляпа
    ...
    cache.HeadToTorso.Color = Settings.Color
    cache.HeadToTorso.Thickness = 1
    cache.TorsoToHips.Color = Settings.Color
    cache.TorsoToHips.Thickness = 1
    cache.LeftArm.Color = Settings.Color
    cache.LeftArm.Thickness = 1
    cache.RightArm.Color = Settings.Color
    cache.RightArm.Thickness = 1
    cache.LeftLeg.Color = Settings.Color
    cache.LeftLeg.Thickness = 1
    cache.RightLeg.Color = Settings.Color
    cache.RightLeg.Thickness = 1
...
local function OnRender()
    ...
    -- Повышаем плавность: 0.03 сек (33 FPS) вместо 0.05
    if tick() - lastUpdate < 0.03 then return end
    lastUpdate = tick()

    for _, player in pairs(Players:GetPlayers()) do
        -- Показываем на всех игроках, включая LocalPlayer, если включено
        if not Settings.ShowOnLocal and player == LocalPlayer then continue end
        ...
        -- ----- НОВЫЙ РАСЧЁТ ВЕРХНЕЙ И НИЖНЕЙ ТОЧКИ -----
        -- Используем стандартный рост Roblox: примерно 5 стадов от ног до макушки.
        -- HumanoidRootPart находится примерно на высоте 2.5 стада от ног.
        -- Верхняя точка: root.Position + Vector3.new(0, 2.5, 0)
        -- Нижняя точка: root.Position - Vector3.new(0, 2.5, 0)
        -- Это даст центрированный бокс.
        local topPos = root.Position + Vector3.new(0, 2.5, 0)
        local bottomPos = root.Position - Vector3.new(0, 2.5, 0)
        ...
        if not topOnScreen or not bottomOnScreen or not onScreen then
            for _, d in pairs(cache) do
                if d and d.Visible ~= nil then
                    d.Visible = false
                end
        ...
        if Settings.Box and cache.Box then
            ...
            cache.Box.Position = Vector2.new(centerX - boxWidth/2, top.Y)
            cache.Box.Color = Settings.Color
        ...
        if Settings.Name and cache.Name then
            ...
            cache.Name.Position = Vector2.new(centerX, top.Y - 18)
            cache.Name.Color = Settings.Color
        ...
        if Settings.Distance and cache.Distance then
            cache.Distance.Position = Vector2.new(centerX, bottom.Y + 4)
        ...
        -- ----- ПОЛНЫЙ СКЕЛЕТ -----
        if Settings.Skeleton and head and cache.HeadToTorso then
            -- Проверяем наличие торса и бёдер
            local torso = player.Character:FindFirstChild("UpperTorso") or player.Character:FindFirstChild("Torso")
            local hips = player.Character:FindFirstChild("LowerTorso") or player.Character:FindFirstChild("HumanoidRootPart")
            ...
            local leftLowerArm = player.Character:FindFirstChild("LeftLowerArm") or leftUpperArm
            local rightLowerArm = player.Character:FindFirstChild("RightLowerArm") or rightUpperArm
            local leftUpperLeg = player.Character:FindFirstChild("LeftUpperLeg")
            local rightUpperLeg = player.Character:FindFirstChild("RightUpperLeg")
            local leftLowerLeg = player.Character:FindFirstChild("LeftLowerLeg") or leftUpperLeg
            local rightLowerLeg = player.Character:FindFirstChild("RightLowerLeg") or rightUpperLeg

            local headPos = Camera:WorldToViewportPoint(head.Position)
            local torsoPos = torso and Camera:WorldToViewportPoint(torso.Position) or headPos
            local hipsPos = hips and Camera:WorldToViewportPoint(hips.Position) or headPos
            ...
            -- Торс -> бёдра
            cache.TorsoToHips.From = Vector2.new(torsoPos.X, torsoPos.Y)
            cache.TorsoToHips.To = Vector2.new(hipsPos.X, hipsPos.Y)
            cache.TorsoToHips.Color = Settings.Color
            cache.TorsoToHips.Visible = true

            -- Левая рука (плечо -> кисть)
            if leftUpperArm and leftLowerArm then
                local shoulder = Camera:WorldToViewportPoint(leftUpperArm.Position)
                local hand = Camera:WorldToViewportPoint(leftLowerArm.Position)
                cache.LeftArm.From = Vector2.new(shoulder.X, shoulder.Y)
                cache.LeftArm.To = Vector2.new(hand.X, hand.Y)
                cache.LeftArm.Color = Settings.Color
                cache.LeftArm.Visible = true
            else
                cache.LeftArm.Visible = false
            end

            -- Правая рука
            if rightUpperArm and rightLowerArm then
                local shoulder = Camera:WorldToViewportPoint(rightUpperArm.Position)
                local hand = Camera:WorldToViewportPoint(rightLowerArm.Position)
                cache.RightArm.From = Vector2.new(shoulder.X, shoulder.Y)
                cache.RightArm.To = Vector2.new(hand.X, hand.Y)
                cache.RightArm.Color = Settings.Color
                cache.RightArm.Visible = true
            else
                cache.RightArm.Visible = false
            end

            -- Левая нога (бедро -> стопа)
            if leftUpperLeg and leftLowerLeg then
                local hip = Camera:WorldToViewportPoint(leftUpperLeg.Position)
                local foot = Camera:WorldToViewportPoint(leftLowerLeg.Position)
                cache.LeftLeg.From = Vector2.new(hip.X, hip.Y)
                cache.LeftLeg.To = Vector2.new(foot.X, foot.Y)
                cache.LeftLeg.Color = Settings.Color
                cache.LeftLeg.Visible = true
            else
                cache.LeftLeg.Visible = false
            end

            -- Правая нога
            if rightUpperLeg and rightLowerLeg then
                local hip = Camera:WorldToViewportPoint(rightUpperLeg.Position)
                local foot = Camera:WorldToViewportPoint(rightLowerLeg.Position)
                cache.RightLeg.From = Vector2.new(hip.X, hip.Y)
                cache.RightLeg.To = Vector2.new(foot.X, foot.Y)
                cache.RightLeg.Color = Settings.Color
                cache.RightLeg.Visible = true
            else
                cache.RightLeg.Visible = false
            end
        else
            -- Скрываем все линии скелета
            if cache.HeadToTorso then cache.HeadToTorso.Visible = false end
            if cache.TorsoToHips then cache.TorsoToHips.Visible = false end
            if cache.LeftArm then cache.LeftArm.Visible = false end
            if cache.RightArm then cache.RightArm.Visible = false end
            if cache.LeftLeg then cache.LeftLeg.Visible = false end
            if cache.RightLeg then cache.RightLeg.Visible = false end
        end

        -- Chine Hat (на всех, включая LocalPlayer, если включено)
...
-- ---- При смене персонажа пересоздаём кэш ----
...
print("Ξ|Ω ESP v5 loaded. Fixes: box position, full skeleton, smoothness.")
