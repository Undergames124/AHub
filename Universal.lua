...
--------------------------------------------------------------------------------
  ...
  FIX 2.6.1     : unpack() -> UNPACK (в Luau нет глобального unpack),
                  ...
                  убран битый loadstring-плейсхолдер со страницы.
--------------------------------------------------------------------------------
...
--==============================================================================
-- 0a. ШИМЫ СОВМЕСТИМОСТИ (исправляет "attempt to call a nil value")
--     Разные исполнители дают разный набор глобалов. Добираем всё, чего нет,
--     ДО того, как код начнёт их вызывать.
--==============================================================================
local UNPACK = unpack or (table and table.unpack)
if not UNPACK then
    -- Ручная реализация: почти не встречается, но чтобы скрипт не падал
    UNPACK = function(t, i, j)
        i, j = i or 1, j or #t
        local out = {}
        for n = i, j do out[#out + 1] = t[n] end
        -- возвращаем через корутину: единственный способ вернуть varargs в чистом Lua 5.1
        local co = coroutine.wrap(function()
            for _, v in ipairs(out) do coroutine.yield(v) end
        end)
        return co()
    end
end

-- table.clear есть только в новых сборках Luau
if not (table and table.clear) then
        ...
        if Config.SilentAim and (m == "FireServer" or m == "InvokeServer") then
            local isShoot = (self == shootRemote) or self.Name:lower():find("shoot")
                ...
                if t then
                    ...
                    -- ИСПРАВЛЕНО: в Luau НЕТ глобального unpack() — только table.unpack.
                    -- Раньше это давало "attempt to call a nil value" при выстреле.
...
function Combat.autoKillStep()
    ...
    for _, p in ipairs(getTargets()) do
        ...
        -- ИСПРАВЛЕНО: вместо `continue` (не поддерживается частью исполнителей:
        -- KRNL, старые Fluxus/Codex) — инвертированное условие
        if not onCd(p) then
...
function ESP.render(dt)
    ...
    for plr in pairs(active) do
        ...
        -- ИСПРАВЛЕНО: вместо `continue` — инвертированное условие
        -- (`continue` ломается на KRNL / старых Fluxus)
