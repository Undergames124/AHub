...
local function FindTarget()
    ...
    if Cfg.Sticky and S.tgt.part and S.tgt.plr then
        ...
        if ch and ch.Parent and GetHP(ch)>0 then
            ...
            if p then
                ...
                else S.tgt.vis=false;return nil,nil end
    ...
    for _,tp in ipairs(Players:GetPlayers()) do
        ...
        if not is360 and GetAng(p)>Cfg.Limits.MaxA then continue end
...
local function FireClick()
    if S.fire.method=="m1" then pcall(function() mouse1press();task.delay(0.05,function() pcall(mouse1release) end) end);return true
    elseif S.fire.method=="vim" then pcall(function() local v=game:GetService("VirtualInputManager");v:SendMouseButtonEvent(Mouse.X,Mouse.Y,0,true,game,0);task.delay(0.05,function() pcall(function() v:SendMouseButtonEvent(Mouse.X,Mouse.Y,0,false,game,0) end) end) end);return true end
    return false
end
local function FireUpdate()
    ...
    for _,tp in ipairs(Players:GetPlayers()) do
        ...
        if SDist(head.Position)<=Cfg.TB.TBFOV then
...
local function TPUpdate()
    ...
    if Cfg.TP.On then
        ...
        pcall(function() Plr.CameraMinZoomDistance=0.5 end)
    end
end
local function TPReset()
    pcall(function() if S.origMaxZoom then Plr.CameraMaxZoomDistance=S.origMaxZoom end end)
    pcall(function() if S.origMinZoom then Plr.CameraMinZoomDistance=S.origMinZoom end end)
    S.origMaxZoom=nil;S.origMinZoom=nil
end

local function SpinUpdate(dt)
    if DEAD or not Cfg.Spin.On or not S.me.alive or not S.me.root or not S.me.hum then return end
    S.spinAng=(S.spinAng+Cfg.Spin.Spd*dt*60)%360
    pcall(function()
        local pos=S.me.root.Position
        local y=S.me.root.CFrame.UpVector
        S.me.root.CFrame=CFrame.new(pos)*CFrame.Angles(0,math.rad(S.spinAng),0)
    end)
end
...
function E.Hide(o)
    ...
    for _,k in ipairs({"box","boxO","name","hpBg","hpFill","tracer","hdot"}) do
        if o[k] then pcall(function() o[k].Visible=false end) end
    end
    if o.cL then for i=1,8 do
...
local WH={}
function WH.Make(uid,ch,isTeam) if DEAD or S.whC[uid] or not ch then return end;local hl=Instance.new("Highlight");hl.Adornee=ch;hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop;hl.FillColor=isTeam and Color3.fromRGB(0,255,0) or Cfg.WH.Fill;hl.OutlineColor=Cfg.WH.Out;hl.FillTransparency=Cfg.WH.FT;hl.OutlineTransparency=Cfg.WH.OT;pcall(function() hl.Parent=ch end);S.whC[uid]=hl end
...
local function Cleanup()
    ...
    pcall(E.DelAll);pcall(WH.KillAll);pcall(HUD.Destroy);pcall(TPReset)
...
local function BuildGUI()
    ...
    local tl=Instance.new("TextLabel",main);tl.Text="XENO v10.1";tl.Size=UDim2.new(1,-50,0,28);tl.Position=UDim2.new(0,10,0,4);tl.BackgroundTransparency=1;tl.TextColor3=MC;tl.Font=Enum.Font.GothamBold;tl.TextSize=SC(15,13);tl.TextXAlignment=Enum.TextXAlignment.Left
    ...
    table.insert(S.conns,RunService.RenderStepped:Connect(function(dt)
        ...
        if Cfg.On and S.me.alive then
            ...
            if part then
                ...
                elseif Cfg.AimMode=="flick" then
                else ApplyAim(part) end
            else
