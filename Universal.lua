...
local Cfg={
    ...
    ESP={
        ...
        Box={On=true,W=SC(1.5,1),CL=0.25,OW=2,Outline=true,VisCheck=false,VisClr=Color3.fromRGB(50,255,50),InvisClr=Color3.fromRGB(255,50,50)},
        Name={On=true,Sz=SC(14,12),Pos="Top",Fmt="Name+Distance",OL=true,Shadow=true},
        HP={On=true,W=3,Off=5,Pos="Left",Txt=true,OL=true,Smooth=true,SmoothSpd=10},
        Dist={On=false,Sz=12},Weapon={On=false,Sz=11},
        Tracer={On=false,W=1.5,Origin="Bottom",Outline=false,OW=1},
        Skel={On=false,W=1.5,VisCheck=false},
        HeadDot={On=false,Rad=SC(3,4),Filled=true,Outline=true,OW=1},
...
local S={
    ...
    botCache={},botTick=0,realPlayers={},realTick=0,
}
...
function Bot.IsRealPlayer(p)
    if not p or p==Plr then return false end
    local children = p:GetChildren()
    if #children == 0 then return false end
    if #children == 1 and children[1].Name == "Av" then return false end
    local hasChar = p.Character and p.Character.Parent and p.Character:FindFirstChildOfClass("Humanoid")
    if hasChar then return true end
    local hasBackpack = p:FindFirstChild("Backpack")
    local hasPlayerGui = p:FindFirstChild("PlayerGui")
    local hasPlayerScripts = p:FindFirstChild("PlayerScripts")
    local hasStarter = p:FindFirstChild("StarterGear")
    local realCount = 0
    for _,c in ipairs(children) do
        if c.Name ~= "Av" then realCount = realCount + 1 end
    end
    if realCount >= 2 then return true end
    if hasBackpack or hasPlayerGui or hasPlayerScripts or hasStarter then return true end
    return false
end

function Bot.RefreshReal()
    if tick() - S.realTick < 0.5 then return end
    S.realTick = tick()
    S.realPlayers = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p == Plr then continue end
        S.realPlayers[p.Name] = Bot.IsRealPlayer(p)
    end
end

function Bot.IsBotPlayer(p)
    if not p or p == Plr then return false end
    Bot.RefreshReal()
    if S.realPlayers[p.Name] == nil then return true end
    return not S.realPlayers[p.Name]
end
...
function Bot.GetAllTargets()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p == Plr then continue end
        local isBot = Bot.IsBotPlayer(p)
        local ch = p.Character
        if not ch or not ch.Parent then
            ch = WS:FindFirstChild(p.Name)
        end
        if not ch or not ch.Parent then continue end
        local alive = Bot.IsAlive(ch)
        if not alive then continue end
        if isBot then
            table.insert(targets, {model=ch, player=p, isBot=true, name="BOT"})
        else
            table.insert(targets, {model=ch, player=p, isBot=false, name=p.DisplayName or p.Name})
        end
    end
    return targets
end

function Bot.CountBots()
    Bot.RefreshReal()
    local count = 0
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Plr and Bot.IsBotPlayer(p) then count = count + 1 end
    end
    return count
end
...
local function SetupChar()
    local function onC(ch)
        ...
        S.me.char=ch;S.me.alive=false;S.tgt.part=nil;S.tgt.plr=nil;S.tgt.lastPos=nil;S.tgt.sVel=Vector3.zero
...
function T.GetP(ch)
    ...
    if Cfg.Part=="Nearest" then
        ...
        end;return b or Res.Root(ch)
    end
    return ch:FindFirstChild(Cfg.Part) or ch:FindFirstChild("Head") or ch:FindFirstChild("UpperTorso") or ch:FindFirstChild("HumanoidRootPart") or Res.Root(ch)
end

function T.ValidTarget(tgt)
    if not tgt or not tgt.model or not tgt.model.Parent then return false end
    local ch=tgt.model;local rp=Res.Root(ch);if not rp then return false end
...
function T.Find()
    ...
    if Cfg.Sticky and S.tgt.part and(S.tgt.plr or S.tgt.isBot) then
        local ch=S.tgt.botModel or(S.tgt.plr and(S.tgt.plr.Character or WS:FindFirstChild(S.tgt.plr.Name)))
        if not ch or not ch.Parent then S.tgt.part=nil;S.tgt.plr=nil;S.tgt.vis=false;S.tgt.isBot=false;S.tgt.botModel=nil
        else local hp=Res.HP(ch);if hp<=0 then S.tgt.part=nil;S.tgt.plr=nil;S.tgt.vis=false;S.tgt.isBot=false;S.tgt.botModel=nil
        else local p=T.GetP(ch);if not p then S.tgt.part=nil;S.tgt.plr=nil;S.tgt.vis=false;S.tgt.isBot=false;S.tgt.botModel=nil
            ...
            else S.tgt.vis=false;return nil,nil,false,nil end end end end
    end
...
function Sil.Install()
    ...
    if hasHM and hasNC then pcall(function()
        local old;old=hookmetamethod(game,"__namecall",wrap(function(self,...) if DEAD then return old(self,...) end;local m=getnamecallmethod();local args={...};if Sil.Should() then local tp=Sil.Pos();if tp then if m=="Raycast" and self==WS and #args>=2 and typeof(args[1])=="Vector3" then S.aim.silentPos=tp;local d=(tp-args[1]);if d.Magnitude>0.001 then d=d.Unit*1000 end;return old(self,args[1],d,select(3,...)) end;if(m=="FindPartOnRay"or m=="FindPartOnRayWithIgnoreList"or m=="FindPartOnRayWithWhitelist")and self==WS and typeof(args[1])=="Ray" then S.aim.silentPos=tp;local d=(tp-args[1].Origin);if d.Magnitude>0.001 then d=d.Unit*1000 end;return old(self,Ray.new(args[1].Origin,d),select(2,...)) end;if(m=="ScreenPointToRay"or m=="ViewportPointToRay")and self==Cam then local sp=W2S(tp);if sp then S.aim.silentPos=tp;return old(self,sp.X,sp.Y) end end end end;return old(self,...) end));hooked=true
    end);pcall(function()
        local old;old=hookmetamethod(game,"__index",wrap(function(self,k) if DEAD then return old(self,k) end;if Sil.Should() and self==Mouse then if k=="Hit" then local cf=Sil.CF();if cf then S.aim.silentPos=cf.Position;return cf end elseif k=="Target" and S.tgt.part then return S.tgt.part elseif k=="UnitRay" then local p=Sil.Pos();if p and Cam then local d=p-Cam.CFrame.Position;if d.Magnitude>0.001 then return Ray.new(Cam.CFrame.Position,d.Unit) end end elseif(k=="X"or k=="Y")then local p=Sil.Pos();if p then local sp=W2S(p);if sp then return k=="X"and sp.X or sp.Y end end end end;return old(self,k) end))
    end) end
...
local function DrawFull(o,bx,by,bw,bh,clr,oClr) for i=1,8 do DV(o.cL[i],false);DV(o.cO[i],false) end;for i=1,12 do DV(o.glow[i],false) end;for i=1,13 do DV(o.cyber[i],false) end;for i=1,8 do DV(o.grad[i],false) end;if IV(o.bO) and Cfg.ESP.Box.Outline then DS(o.bO,"Size",Vector2.new(bw+4,bh+4));DS(o.bO,"Position",Vector2.new(bx-2,by-2));DS(o.bO,"Color",oClr);DS(o.bO,"Thickness",Cfg.ESP.Box.W);DV(o.bO,true) else DV(o.bO,false) end;if IV(o.bI) then DS(o.bI,"Size",Vector2.new(bw,bh));DS(o.bI,"Position",Vector2.new(bx,by));DS(o.bI,"Color",clr);DS(o.bI,"Thickness",Cfg.ESP.Box.W);DV(o.bI,true) end end
local function DrawGlow(o,bx,by,bw,bh,clr) for i=1,8 do DV(o.cL[i],false);DV(o.cO[i],false) end;DV(o.bI,false);DV(o.bO,false);for i=1,13 do DV(o.cyber[i],false) end;for i=1,8 do DV(o.grad[i],false) end;local G=Cfg.ESP.Glow;local sides={{Vector2.new(bx,by),Vector2.new(bx+bw,by)},{Vector2.new(bx+bw,by),Vector2.new(bx+bw,by+bh)},{Vector2.new(bx,by+bh),Vector2.new(bx+bw,by+bh)},{Vector2.new(bx,by),Vector2.new(bx,by+bh)}};local idx=1;for layer=1,math.min(G.Layers,3) do local tr=G.BaseTrans+G.FadeStep*(layer-1);local thick=Cfg.ESP.Box.W+G.Spread*layer;for s=1,4 do if idx<=12 and IV(o.glow[idx]) then DS(o.glow[idx],"From",sides[s][1]);DS(o.glow[idx],"To",sides[s][2]);DS(o.glow[idx],"Color",clr);DS(o.glow[idx],"Thickness",thick);DS(o.glow[idx],"Transparency",1-tr);DV(o.glow[idx],true) end;idx=idx+1 end end;for i=idx,12 do DV(o.glow[i],false) end end
local function DrawCyber(o,bx,by,bw,bh,clr,accent) for i=1,8 do DV(o.cL[i],false);DV(o.cO[i],false) end;DV(o.bI,false);DV(o.bO,false);for i=1,12 do DV(o.glow[i],false) end;for i=1,8 do DV(o.grad[i],false) end;local CB=Cfg.ESP.Cyber;local cl=math.max(bw,bh)*0.3;local pts={{Vector2.new(bx,by),Vector2.new(bx+cl,by)},{Vector2.new(bx,by),Vector2.new(bx,by+cl)},{Vector2.new(bx+bw,by),Vector2.new(bx+bw-cl,by)},{Vector2.new(bx+bw,by),Vector2.new(bx+bw,by+cl)},{Vector2.new(bx,by+bh),Vector2.new(bx+cl,by+bh)},{Vector2.new(bx,by+bh),Vector2.new(bx,by+bh-cl)},{Vector2.new(bx+bw,by+bh),Vector2.new(bx+bw-cl,by+bh)},{Vector2.new(bx+bw,by+bh),Vector2.new(bx+bw,by+bh-cl)}};for i=1,8 do if IV(o.cyber[i]) then DS(o.cyber[i],"From",pts[i][1]);DS(o.cyber[i],"To",pts[i][2]);DS(o.cyber[i],"Color",clr);DS(o.cyber[i],"Thickness",2);DV(o.cyber[i],true) end end;if CB.TickMarks then local tl=CB.TickLen;local mx=bx+bw/2;local my=by+bh/2;local ticks={{Vector2.new(mx-tl/2,by),Vector2.new(mx+tl/2,by)},{Vector2.new(mx-tl/2,by+bh),Vector2.new(mx+tl/2,by+bh)},{Vector2.new(bx,my-tl/2),Vector2.new(bx,my+tl/2)},{Vector2.new(bx+bw,my-tl/2),Vector2.new(bx+bw,my+tl/2)}};for i=1,4 do local idx=8+i;if IV(o.cyber[idx]) then DS(o.cyber[idx],"From",ticks[i][1]);DS(o.cyber[idx],"To",ticks[i][2]);DS(o.cyber[idx],"Color",accent);DS(o.cyber[idx],"Thickness",1);DV(o.cyber[idx],true) end end else for i=9,12 do DV(o.cyber[i],false) end end;if CB.ScanLine then o.scanY=(o.scanY or 0)+CB.ScanSpeed;if o.scanY>bh then o.scanY=0 end;if IV(o.cyber[13]) then DS(o.cyber[13],"From",Vector2.new(bx,by+o.scanY));DS(o.cyber[13],"To",Vector2.new(bx+bw,by+o.scanY));DS(o.cyber[13],"Color",accent);DS(o.cyber[13],"Thickness",1);DS(o.cyber[13],"Transparency",0.5);DV(o.cyber[13],true) end else DV(o.cyber[13],false) end end
...
function E.Render(key,ch,displayName,isBot,isTeam,dt)
    ...
    if isBot and not Cfg.ESP.Bots.On then E.HideAll(o);return end
    ...
    if isBot then boxClr=CC2.BotBox;boxBot=CC2.BotBoxBot;nameClr=CC2.BotName;glowClr=CC2.BotGlow;cyberClr=CC2.BotCyber;cyberAcc=CC2.BotCyberAccent;tracerClr=CC2.BotTracer;skelClr=CC2.BotSkel
    elseif isTeam then boxClr=CC2.TBox;boxBot=CC2.TBoxBot;nameClr=CC2.TName;glowClr=CC2.TGlow;cyberClr=CC2.TCyber;cyberAcc=CC2.TCyberAccent;tracerClr=CC2.TTracer;skelClr=CC2.TSkel
    else boxClr=CC2.EBox;boxBot=CC2.EBoxBot;nameClr=CC2.EName;glowClr=CC2.EGlow;cyberClr=CC2.ECyber;cyberAcc=CC2.ECyberAccent;tracerClr=CC2.ETracer;skelClr=CC2.ESkel end
    if Cfg.ESP.Box.VisCheck then boxClr=o.isVis and Cfg.ESP.Box.VisClr or Cfg.ESP.Box.InvisClr end
    if Cfg.ESP.Box.On then local st=Cfg.ESP.Style;if st=="Corner" then DrawCorner(o,bx,by,bw,bh,boxClr,CC2.Outline) elseif st=="Full" then DrawFull(o,bx,by,bw,bh,boxClr,CC2.Outline) elseif st=="Glow" then DrawGlow(o,bx,by,bw,bh,glowClr) elseif st=="Cyber" then DrawCyber(o,bx,by,bw,bh,cyberClr,cyberAcc) elseif st=="Gradient" then DrawGradient(o,bx,by,bw,bh,boxClr,boxBot) else DrawCorner(o,bx,by,bw,bh,boxClr,CC2.Outline) end else HideBox(o) end
    local topTY=by;local botTY=by+bh+2;local rightTY=by
    if Cfg.ESP.Name.On then local txt;if isBot then txt=Cfg.ESP.Name.Fmt=="Name+Distance" and string.format("BOT [%.0fm]",dist) or "BOT" else txt=Cfg.ESP.Name.Fmt=="Name+Distance" and string.format("%s [%.0fm]",displayName,dist) or displayName end;local nPos;if Cfg.ESP.Name.Pos=="Top" then topTY=topTY-Cfg.ESP.Name.Sz-2;nPos=Vector2.new(bx+bw/2,topTY);ShowText(o.nT,nPos,txt,nameClr,Cfg.ESP.Name.Sz,true,Cfg.ESP.Name.OL) elseif Cfg.ESP.Name.Pos=="Bottom" then nPos=Vector2.new(bx+bw/2,botTY);ShowText(o.nT,nPos,txt,nameClr,Cfg.ESP.Name.Sz,true,Cfg.ESP.Name.OL);botTY=botTY+Cfg.ESP.Name.Sz+2 else nPos=Vector2.new(bx+bw+4,rightTY);ShowText(o.nT,nPos,txt,nameClr,Cfg.ESP.Name.Sz,false,Cfg.ESP.Name.OL);rightTY=rightTY+Cfg.ESP.Name.Sz+2 end;if Cfg.ESP.Name.Shadow then ShowText(o.nSh,nPos+Vector2.new(1,1),txt,CC2.NameShadow,Cfg.ESP.Name.Sz,Cfg.ESP.Name.Pos~="Right",false) else HideText(o.nSh) end else HideText(o.nT);HideText(o.nSh) end
    ...
    if Cfg.ESP.Flags.On then local ps={};if isBot then table.insert(ps,"BOT") end;if Cfg.ESP.Flags.ShowDist then table.insert(ps,string.format("%.0fm",dist)) end;if Cfg.ESP.Flags.ShowVis then table.insert(ps,o.isVis and"VIS"or"INVIS") end;local str=table.concat(ps," | ");if #str>0 then ShowText(o.flT,Vector2.new(bx+bw+4,rightTY),str,CC2.FlagsClr,Cfg.ESP.Flags.Sz,false,true) else HideText(o.flT) end else HideText(o.flT) end
    if Cfg.ESP.HP.On then local pct=math.clamp(hp/math.max(mhp,1),0,1);if Cfg.ESP.HP.Smooth then if not o.smoothHP then o.smoothHP=pct end;o.smoothHP=o.smoothHP+(pct-o.smoothHP)*math.clamp(Cfg.ESP.HP.SmoothSpd*dt,0,1);pct=o.smoothHP end;local hc=HPCol(pct);local hP,bW,off=Cfg.ESP.HP.Pos,Cfg.ESP.HP.W,Cfg.ESP.HP.Off;local bgX,bgY,bgW,bgH,fX,fY,fW,fH;if hP=="Left" then bgX=bx-off-bW-1;bgY=by-1;bgW=bW+2;bgH=bh+2;fH=math.max(bh*pct,1);fX=bgX+1;fY=by+bh-fH;fW=bW elseif hP=="Right" then bgX=bx+bw+off-1;bgY=by-1;bgW=bW+2;bgH=bh+2;fH=math.max(bh*pct,1);fX=bgX+1;fY=by+bh-fH;fW=bW else bgX=bx-1;bgY=by+bh+off-1;bgW=bw+2;bgH=bW+2;fX=bgX+1;fY=bgY+1;fW=math.max(bw*pct,1);fH=bW end;if IV(o.hBg) then DS(o.hBg,"Position",Vector2.new(bgX,bgY));DS(o.hBg,"Size",Vector2.new(math.max(bgW,1),math.max(bgH,1)));DS(o.hBg,"Color",CC2.HPBg);pcall(function() o.hBg.Visible=true end) end;if IV(o.hBo) then if Cfg.ESP.HP.OL then DS(o.hBo,"Position",Vector2.new(bgX-1,bgY-1));DS(o.hBo,"Size",Vector2.new(math.max(bgW+2,1),math.max(bgH+2,1)));pcall(function() o.hBo.Visible=true end) else DV(o.hBo,false) end end;if IV(o.hF) then DS(o.hF,"Position",Vector2.new(fX,fY));DS(o.hF,"Size",Vector2.new(math.max(fW,1),math.max(fH,1)));DS(o.hF,"Color",hc);pcall(function() o.hF.Visible=true end) end;if IV(o.hT) then if Cfg.ESP.HP.Txt and hp<mhp then local htPos;if hP=="Left"or hP=="Right" then htPos=Vector2.new(bgX+bgW/2,fY-14);ShowText(o.hT,htPos,string.format("%.0f",hp),hc,11,true,true) else htPos=Vector2.new(fX+fW+3,fY-2);ShowText(o.hT,htPos,string.format("%.0f",hp),hc,11,false,true) end else HideText(o.hT) end end else DV(o.hBg,false);DV(o.hBo,false);DV(o.hF,false);HideText(o.hT) end
...
function E.UpdateAll(dt)
    ...
    local allTargets=Bot.GetAllTargets()
    for _,tgt in ipairs(allTargets) do
        local key=(tgt.isBot and "bot_" or "p_")..tgt.player.UserId
        activeKeys[key]=true
        if not S.espC[key] then E.New(key) end
        local isTeam=not tgt.isBot and TeamEq(Plr,tgt.player)
...
local WH={}
function WH.Make(key,ch,isBot,isTeam) if DEAD or S.whC[key] or not ch or not ch.Parent then return end;local hl=Instance.new("Highlight");hl.Name="XWH";hl.Adornee=ch;hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop;hl.Enabled=true;if isBot then hl.FillColor=Cfg.WH.BotFill;hl.OutlineColor=Cfg.WH.BotOut elseif isTeam then hl.FillColor=Cfg.WH.TFill;hl.OutlineColor=Cfg.WH.TOut else hl.FillColor=Cfg.WH.Fill;hl.OutlineColor=Cfg.WH.Out end;hl.FillTransparency=Cfg.WH.FT;hl.OutlineTransparency=Cfg.WH.OT;pcall(function() hl.Parent=ch end);S.whC[key]=hl end
...
function WH.UpdateAll()
    ...
    local allTargets=Bot.GetAllTargets()
    for _,tgt in ipairs(allTargets) do
        if tgt.isBot and not Cfg.WH.ShowBots then continue end
        local isTeam=not tgt.isBot and TeamEq(Plr,tgt.player)
        if isTeam and not Cfg.WH.ShowTeam then continue end
        local key=(tgt.isBot and "bot_" or "p_")..tgt.player.UserId
        activeKeys[key]=true
        if not S.whC[key] then WH.Make(key,tgt.model,tgt.isBot,isTeam) end
    end
...
local function Cleanup() DEAD=true;task.wait(0.05);pcall(function() if S.aim.flickOn and S.aim.flickCF and Cam then Cam.CFrame=S.aim.flickCF end end);pcall(function() if mouse1release then mouse1release() end end);for i,c in ipairs(S.conns) do pcall(function() c:Disconnect() end) end;S.conns={};pcall(E.DelAll);pcall(WH.KillAll);pcall(HUD.Destroy);pcall(function() CloseDD();CloseSub();if S.mobFrame then S.mobFrame:Destroy() end;S.mobFrame=nil;if S.gui then S.gui:Destroy() end;S.gui=nil end);pcall(Exp.RSpin);pcall(Exp.RTP);_G.XenoLoaded=false;_G.XenoCleanup=nil end
...
function G.Create()
    ...
    G.Tog(sc,"ESP Bots",Cfg.ESP.Bots.On,y,function(v) Cfg.ESP.Bots.On=v end);y=y+TH
    ...
    Lbl("Bots: "..Bot.CountBots().." found",y,GC.org);y=y+SC(20,24)
        ...
        pcall(function()
            ...
            if Cfg.On and S.me.alive then
                ...
                if part then
                    ...
                    local ch=botModel or(plr and(plr.Character or WS:FindFirstChild(plr.Name)))
                    ...
                    if not S.tgt.plr and not S.tgt.isBot then S.tgt.part=nil;S.tgt.name="";S.tgt.lastPos=nil;S.tgt.sVel=Vector3.zero;S.aim.silentPos=nil end;S.tgt.vis=false
                end
            else S.aim.silentPos=nil end
            Flk.Update();Fire.Update();HUD.Update()
            if Cfg.ESP.On then E.UpdateAll(dt) else local keys={};for k in pairs(S.espC) do table.insert(keys,k) end;for _,k in ipairs(keys) do if S.espC[k] then E.HideAll(S.espC[k]) end end end
...
table.insert(S.conns,Players.PlayerRemoving:Connect(function(p) if DEAD then return end;pcall(E.Del,"p_"..p.UserId);pcall(E.Del,"bot_"..p.UserId);pcall(WH.Kill,"p_"..p.UserId);pcall(WH.Kill,"bot_"..p.UserId) end))
SetupChar();task.wait(0.5);Fire.Init();HUD.Create()
if Exec.canSilent and Cfg.Silent.On then Sil.Install() end
G.Create();Input();Loop()
Notify("Xeno v9.6",string.format("%s | %s | Bots:%d",Cfg.AimMode:upper(),IsMobile and"Mobile"or"PC",Bot.CountBots()),5)
